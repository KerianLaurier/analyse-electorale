...
test("la landing s'affiche en mode pré-lancement", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1 })).toContainText(
    /intelligence électorale/i,
  );
  // Pré-lancement : le CTA mène à la liste d'attente, plus à l'essai gratuit.
  await expect(
    page.getByRole("link", { name: /liste d'attente/i }).first(),
  ).toBeVisible();
...
test("la liste d'attente accepte une adresse et répond", async ({ page }) => {
  ...
  await expect(
    section.getByRole("heading", { name: /la plateforme ouvre bientôt/i }),
  ).toBeVisible();
  // Mention RGPD obligatoire : la base légale de la collecte est le consentement.
  await expect(
    section.getByText(/politique de confidentialité/i),
  ).toBeVisible();
...
test("un échec de connexion affiche une erreur en français", async ({
  page,
}) => {
...
test("l'écran mot de passe oublié s'affiche, l'inscription renvoie à la liste d'attente", async ({
  page,
}) => {
  await page.goto("/auth/login");
  ...
  await expect(page).toHaveURL(/\/#bientot$/);
  await expect(
    page.getByRole("heading", { name: "Démarrer l'essai gratuit" }),
  ).toHaveCount(0);
});

test("la page de secours hors-ligne et le manifest PWA sont servis", async ({
  page,
  request,
}) => {
...
test("la page abonnement n'est plus publique en pré-lancement", async ({
  page,
}) => {
...
test("les modules du worker cartographique sont servis sans authentification", async ({
  request,
}) => {
  ...
  for (const name of ["maplibre-gl-worker.mjs", "maplibre-gl-shared.mjs"]) {
    const response = await request.get(`/maplibre/${version}/${name}`, {
      maxRedirects: 0,
    });
  ...
  const contour = await request.get("/france_contour.geojson", {
    maxRedirects: 0,
  });
  expect(contour.status()).toBe(200);
  expect((await contour.json()).type).toBe("FeatureCollection");
});
