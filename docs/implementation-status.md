...
| 4 : dépendances | Versions corrigées dans le lockfile, Next 16.3.4, MapLibre 6.4.1. Worker ESM et module partagé copiés automatiquement avant dev/build ; ressources publiques exclues du proxy. Cache persistant du build désactivé et anciens caches purgés ; contrôle CI d’absence d’une valeur serveur fictive dans les artefacts. | WebGL 2 requis par MapLibre 6. Vérification des navigateurs cibles à compléter. |
...
| 14, 23 : droits réels | Référence des 19 tables initiales ; migrations rejouées sur cette structure avec RLS et droits de colonnes. Propriété immuable, liens parent/enfant contrôlés, accès conditionné au statut courant, transfert d’équipe encadré, édition du plan réservée au propriétaire. | Types Supabase générés et revue de l’exposition des colonnes de profil entre membres restent à traiter. |
| Sièges Équipe | Un abonnement couvre cinq personnes, titulaire inclus. Invitations sérialisées, droits effectifs en base, blocage du second Checkout et couverture affichée dans le compte. Propriété opérationnelle et titulaire de facturation séparés. | Auditer les équipes existantes avant migration ; reprise de facturation entre titulaires à organiser avec le support. Essai : cinq sièges ; Cabinet : illimité. |
...
Les dix migrations de septembre doivent être appliquées dans l’ordre avec suivi de leur version. Le bundle remis dans les livrables les rassemble dans une transaction avec délais de verrouillage/exécution bornés. Ne pas rejouer les anciennes migrations ni la référence initiale sur la production.

Les droits partagés dépendent de `workspace_entitlement`, appelée par l’identité, les pages de compte et le Checkout. La nouvelle administration appelle `admin_accounts_page` et une route utilisant l’API Auth officielle. L’ancien appel `admin_create_account` est révoqué. Le Checkout et le webhook appellent également les nouvelles fonctions. Préparer le frontend et les variables serveur avant la fenêtre de bascule, appliquer le SQL, puis déployer la version correspondante. Un déploiement du frontend seul n’est pas compatible.
