...
vi.mock("@/lib/supabase/admin", () => ({
  ...
  createServiceClient: () => ({
    rpc: mock.rpc,
    from: () => ({
      select: () => ({ eq: () => ({ maybeSingle: mock.maybeSingle }) }),
    }),
  }),
}));
...
beforeEach(() => {
  ...
  mock.rpc
    ...
    .mockImplementation(async (name: string) => ({
      data: name === "acquire_stripe_sync" ? "lease_1" : true,
      error: null,
    }));
...
describe("webhook signé", () => {
  ...
  it("retourne un succès pour un doublon déjà validé en base", async () => {
    mock.rpc.mockImplementation(async (name: string) => ({
      data: name === "acquire_stripe_sync" ? "lease_1" : false,
      error: null,
    }));
  ...
  it("un ancien snapshot actif relit l'état Stripe désormais annulé", async () => {
    ...
    expect(
      (await POST(request("customer.subscription.updated", subscription)))
        .status,
    ).toBe(200);
    expect(mock.rpc).toHaveBeenCalledWith(
      "apply_stripe_event_serialized",
  ...
  it("une panne de réconciliation ne devient pas un client inconnu acquitté", async () => {
    ...
    expect(
      (
        await POST(
          request("invoice.payment_failed", {
            customer: "cus_1",
            metadata: {},
          }),
        )
