...
const mock = vi.hoisted(() => ({
  user: { id: "user_1", email: "test@example.test" } as { id: string; email: string } | null,
  profile: vi.fn(), attach: vi.fn(), customer: vi.fn(), checkout: vi.fn(), portal: vi.fn(),
}));
...
vi.mock("@/lib/supabase/admin", () => ({
  ...
  createServiceClient: () => ({ from: () => ({
    ...
    update: () => ({ eq: () => ({ select: () => ({ single: mock.attach }) }) }),
  }) }),
}));
vi.mock("@/lib/stripe", () => ({
  stripeEnabled: () => true,
  ...
  getStripe: () => ({
    customers: { create: mock.customer }, checkout: { sessions: { create: mock.checkout } },
...
const profile = { subscription_status: "trial", stripe_customer_id: null, stripe_subscription_id: null };
function request(body: unknown = { tier: "equipe", cycle: "monthly" }) {
  return new Request("https://app.example.test/api/stripe/checkout", {
    method: "POST", headers: { Origin: "https://untrusted.example.test" }, body: JSON.stringify(body),
  });
}
beforeEach(() => {
...
describe("création d'un paiement", () => {
  ...
  it.each([null, [], "invalid", { tier: "parti", cycle: "monthly" }])("refuse le corps invalide %j", async body => {
  ...
  it("ne lance aucun paiement si le rattachement du client échoue", async () => {
    mock.attach.mockResolvedValue({ data: null, error: { message: "database unavailable" } });
  ...
  it("réutilise une clé client stable et ignore l'en-tête Origin", async () => {
    ...
    expect(mock.customer).toHaveBeenCalledWith(expect.any(Object), { idempotencyKey: "customer:user_1" });
    expect(mock.checkout).toHaveBeenCalledWith(expect.objectContaining({
      success_url: "https://app.example.test/auth/abonnement?checkout=success",
      cancel_url: "https://app.example.test/auth/abonnement?checkout=cancelled",
    }));
  });
  it("redirige la gestion d'un abonnement actif vers le portail", async () => {
    mock.profile.mockResolvedValue({ data: { ...profile, subscription_status: "active", stripe_subscription_id: "sub_1" }, error: null });
  ...
  it("le portail utilise aussi l'origine serveur et le compte connecté", async () => {
    mock.profile.mockResolvedValue({ data: { ...profile, stripe_customer_id: "cus_1", stripe_subscription_id: "sub_1" }, error: null });
    expect((await portal(request({ flow: "subscription_update" }))).status).toBe(200);
    expect(mock.portal).toHaveBeenCalledWith(expect.objectContaining({ customer: "cus_1", return_url: "https://app.example.test/auth/abonnement" }));
  });
  it("ne lance pas le portail si la lecture du profil échoue", async () => {
    mock.profile.mockResolvedValue({ data: null, error: { message: "timeout" } });
