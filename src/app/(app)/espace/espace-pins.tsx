...
import { usePins, useLoadState, PIN_TYPE_LABELS, type Pin, type PinType } from "@/lib/pins";
...
export function EspacePins() {
  ...
  if (error) return <ErrorState message="Impossible de charger les épingles." onRetry={retry} />;
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-foreground/10 bg-surface/60 px-6 py-16 text-center">
        ...
        <p className="max-w-md text-[13px] text-muted-foreground">
          Parcourez une commune, une circonscription, un bureau ou une personne, puis cliquez sur
          l’étoile « Épingler » pour la retrouver ici — et la partager avec votre équipe.
        </p>
        <Link
          href="/explorer"
    ...
    <div className="group flex items-center gap-3 rounded-lg border border-foreground/5 bg-surface p-3.5 shadow-card transition-colors hover:border-warm/40">
      <Link href={pin.href} className="flex min-w-0 flex-1 items-center gap-3">
        <div className="min-w-0">
          ...
          {pin.sublabel && <p className="truncate text-[11.5px] text-muted-foreground">{pin.sublabel}</p>}
        </div>
      </Link>
