...
import { EspaceOnboarding } from "@/app/(app)/espace/espace-onboarding";
 ...
 *
 * Adapté au rôle : un responsable voit le pilotage de la campagne (avancement
 * de l'objectif, couverture des secteurs, qui fait quoi) ; un membre voit
 * d'abord ce qui lui est assigné — ses actions et ses permanences.
 */
export function EspaceToday() {
  ...
  // Vue « membre » : ce qui m'est assigné, plutôt que l'activité de toute
  // l'équipe. Calculé dans tous les cas — un hook ne peut pas être conditionnel.
  const mine = useMemo(
    () => ({
      tasks: active.filter((t) => t.assignee === ctx.meId).sort((a, b) => (a.dueDate ?? "9") < (b.dueDate ?? "9") ? -1 : 1).slice(0, 5),
      shifts: shifts.filter((s) => s.date >= today && s.joined).slice(0, 5),
    }),
    [active, shifts, today, ctx.meId],
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      <EspaceOnboarding ctx={ctx} campaign={campaign} sectors={sectors} goal={goal} />
      ...
      <div className="anim-stagger grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <Stat icon={ListTodo} label={isOwner ? "Actions en cours" : "Mes actions"} value={isOwner ? active.length : mine.tasks.length} href={ESPACE.terrain("actions")} accent />
        <Stat icon={CalendarClock} label={isOwner ? "Permanences à venir" : "Mes permanences"} value={isOwner ? upcomingShifts.length : mine.shifts.length} href={ESPACE.terrain("permanences")} />
        <Stat icon={DoorOpen} label="Personnes contactées" value={field.contacted} href={ESPACE.terrain("porte-a-porte")} />
        <Stat icon={Contact} label="Contacts" value={contacts.length} href={ESPACE.equipe("contacts")} />
        <Stat icon={StickyNote} label="Notes" value={notes.length} href={ESPACE.equipe("notes")} />
        <Stat icon={Star} label="Épingles" value={pins.length} href={ESPACE.plan("epingles")} />
      </div>

      {/* Sondage terrain (porte-à-porte + phoning) */}
      {field.opinions > 0 && (
        <Link
          href={ESPACE.terrain("porte-a-porte")}
          className="block rounded-lg border border-foreground/5 bg-surface p-4 text-left shadow-card transition-colors hover:border-warm/30"
        >
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {/* Échéances proches */}
        <Panel title={isOwner ? "Échéances proches" : "Mes actions"} icon={CalendarClock} href={ESPACE.terrain("actions")}>
          {(isOwner ? upcoming : mine.tasks).length === 0 ? (
            <Empty>{isOwner ? "Aucune échéance planifiée." : "Aucune action ne vous est assignée."}</Empty>
          ) : (
            <ul className="flex flex-col divide-y divide-border/60">
              {(isOwner ? upcoming : mine.tasks).map((t) => (
        ...
        <Panel title={isOwner ? "Prochaines permanences" : "Mes permanences"} icon={Clock} href={ESPACE.terrain("permanences")}>
          {(isOwner ? upcomingShifts : mine.shifts).length === 0 ? (
            <Empty>{isOwner ? "Aucune permanence planifiée." : "Vous n’êtes inscrit·e à aucune permanence."}</Empty>
          ) : (
            <ul className="flex flex-col divide-y divide-border/60">
              {(isOwner ? upcomingShifts : mine.shifts).map((s) => (
      ...
      {/* Notes récentes */}
      <Panel title="Notes récentes" icon={StickyNote} href={ESPACE.equipe("notes")}>
        {notes.length === 0 ? (
      ...
      {ctx.teamId && ctx.members.length > 0 && (
        <section className="rounded-lg border border-foreground/5 bg-surface p-4 shadow-card">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Link href={ESPACE.equipe("membres")} className="inline-flex items-center gap-1 text-[11.5px] font-medium text-warm hover:underline">
  ...
  return (
    <section className="rounded-lg border border-foreground/5 bg-surface p-4 shadow-card">
      <div className="mb-2 flex items-center justify-between">
        ...
        <Link href={href} className="inline-flex items-center gap-1 text-[11.5px] font-medium text-warm hover:underline">
