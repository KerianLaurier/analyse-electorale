...
const ELECTIONS = (Object.keys(SCRUTIN_META) as Scrutin[]).filter(
  (s) =>
    SCRUTIN_META[s].family !== "sociologie" &&
    SCRUTIN_META[s].mailles.includes("circonscriptions"),
);
...
export function PotentielView() {
  ...
  const model = useMemo(
    () =>
      features.data && target.size > 0
        ? ridgeResiduals(features.data, target)
        : { rows: [], r2: 0, n: 0 },
    [features.data, target],
  );

  const sorted = useMemo(
    () => [...model.rows].sort((a, b) => b.residual - a.residual),
    [model.rows],
  );
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-[12px] text-muted-foreground">
            Score d&apos;un bloc modélisé par la sociologie (régression sur 10
            indicateurs INSEE) → sur / sous-performance par circonscription.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {isLoading && (
            <Spinner currentColor className="size-4 text-muted-foreground" />
          )}
          {model.rows.length > 0 && (
            <ExportButton onClick={exportCsv} label="Exporter le ciblage" />
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg bg-surface p-4 shadow-card">
        <Select
          ...
          onValueChange={(appicaValue) =>
            setScrutin(String(appicaValue ?? "") as Scrutin)
          }
          size="sm"
        >
          <SelectTrigger className="rounded-pill bg-primary text-[12px] font-medium text-primary-foreground">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {ELECTIONS.map((s) => (
        ...
        <div className="inline-flex items-center gap-2 rounded-pill border border-border bg-surface px-2.5 py-1">
          ...
          <Select
            value={blocId}
            onValueChange={(appicaValue) =>
              setBlocId(String(appicaValue ?? "") as BlocId)
            }
            size="sm"
          >
            <SelectTrigger className="bg-transparent text-[12px] font-medium text-foreground/80">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {BLOCS.map((b) => (
      ...
      <div className="anim-stagger grid grid-cols-4 gap-2">
        ...
        <KPICard
          label="Ajustement aux données observées (R²)"
          value={model.n ? `${Math.round(model.r2 * 100)} %` : "—"}
          hint="Ajustement sur les données d’entraînement ; ne mesure pas la précision sur de nouvelles données."
        />
        <KPICard
          label="Sur-performance max"
          value={sorted[0] ? fmtPts(sorted[0].residual) : "—"}
          hint={sorted[0] ? libelles.get(sorted[0].code) : undefined}
          accent="over"
        />
        <KPICard
          label="Sous-performance max"
          value={
            sorted.length ? fmtPts(sorted[sorted.length - 1].residual) : "—"
          }
          hint={
            sorted.length
              ? libelles.get(sorted[sorted.length - 1].code)
              : undefined
          }
          accent="under"
        />
      ...
      <div className="grid min-h-[460px] grid-cols-[1fr_1fr_420px] gap-2">
        <PerfList
          title="Bastions"
          subtitle="sur-performance vs profil"
          icon={TrendingUp}
          color={OVER}
          rows={over}
          libelles={libelles}
        ...
        />
        <PerfList
          title="Réserves de voix"
          subtitle="sous-performance vs profil"
          icon={TrendingDown}
          color={UNDER}
          rows={under}
          libelles={libelles}
        ...
        <div className="flex flex-col overflow-hidden rounded-lg bg-surface shadow-card">
          <div className="border-b border-border/60 px-4 py-3">
            ...
            <p className="mt-0.5 text-[13px] font-medium">
              Écart au potentiel sociologique
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 border-t border-border/60 px-4 py-2 text-[10.5px] text-muted-foreground">
            ...
            <span className="ml-auto text-[10px] text-muted-foreground/70">
              {bloc.label} · {SCRUTIN_META[scrutin].short}
            </span>
          </div>
        </div>
      </div>
      ...
      <p className="px-2 text-[10.5px] text-muted-foreground/70">
        Modèle indicatif : on régresse la part du bloc sur 10 indicateurs INSEE
        (revenus, inégalités, âge, chômage, CSP, diplômes) agrégés par
        circonscription. Le résidu (sur / sous-performance) reflète tout ce que
        la sociologie n&apos;explique pas (implantation, candidat, dynamique
        locale). Corrélation ≠ causalité.
      </p>
    </div>
  );
}
...
function PerfList({
  ...
  const shown = rows.filter((r) =>
    positive ? r.residual > 0 : r.residual < 0,
  );
  return (
            ...
            {shown.map((r) => (
              ...
              >
                <span
                  ...
                  title={libelles.get(r.code) ?? r.code}
                >
