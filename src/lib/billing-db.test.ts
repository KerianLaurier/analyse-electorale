...
beforeAll(async () => {
  ...
  `);
  for (const name of ["20260910200000_atomic_stripe_events.sql", "20260910201000_restrict_invoice_billing.sql"]) {
    await db.exec(await readFile(`supabase/migrations/${name}`, "utf8"));
  }
}, 30_000);

beforeEach(async () => {
  await db.exec("reset role; truncate profiles, stripe_events, billing_events, invoice_billing_accounts cascade;");
  await db.query("insert into profiles(id) values ($1)", [USER]);
  await db.query("select set_config('request.jwt.claim.sub', $1, false)", [USER]);
});
afterAll(async () => { await db?.close(); });

const patch = { subscription_status: "active", subscription_tier: "equipe", billing_cycle: "monthly", stripe_subscription_id: "sub_new", trial_ends_at: null, cancel_at: null };
const bill = { type: "subscribe", tier: "equipe", cycle: "monthly", details: {} };
const apply = (id = "evt_1", body = patch, event = bill, sub = "sub_new", user = USER) => db.query<{ applied: boolean }>(
...
describe("transaction de paiement", () => {
  it("enregistre une fois le profil et le journal, même après rejeu", async () => {
    ...
    expect((await db.query("select count(*)::int n from billing_events")).rows).toEqual([{ n: 1 }]);
    expect((await db.query("select subscription_status from profiles")).rows).toEqual([{ subscription_status: "active" }]);
  });
  it("annule aussi la déduplication et le profil si le journal échoue", async () => {
    await expect(apply("evt_1", patch, { ...bill, type: "invalid_type" })).rejects.toThrow();
    expect((await db.query("select count(*)::int n from stripe_events")).rows).toEqual([{ n: 0 }]);
    expect((await db.query("select subscription_status from profiles")).rows).toEqual([{ subscription_status: "trial" }]);
    expect((await apply()).rows[0].applied).toBe(true);
  });
  it("ne perd pas un événement quand le profil n'existe pas", async () => {
    await expect(apply("evt_missing", patch, bill, "sub_new", "00000000-0000-0000-0000-000000000002")).rejects.toThrow("Profil Stripe introuvable");
    expect((await db.query("select count(*)::int n from stripe_events")).rows).toEqual([{ n: 0 }]);
  });
  it("une ancienne résiliation ne coupe pas le nouvel abonnement", async () => {
    await apply();
    await apply("evt_old_cancel", { ...patch, subscription_status: "inactive", stripe_subscription_id: null as unknown as string }, { ...bill, type: "cancel" }, "sub_old");
    expect((await db.query("select subscription_status, stripe_subscription_id from profiles")).rows).toEqual([{ subscription_status: "active", stripe_subscription_id: "sub_new" }]);
  });
...
describe("facturation sur autorisation", () => {
  it("interdit à un compte ordinaire de s'activer gratuitement", async () => {
    await db.exec("set role authenticated");
    await expect(db.query("select self_set_plan('equipe', 'monthly')")).rejects.toThrow(/non autorisée/);
  });
  it("autorise les clients facture explicitement inscrits par un administrateur", async () => {
    await db.query("insert into invoice_billing_accounts(user_id) values ($1)", [USER]);
    await db.exec("set role authenticated");
    await db.query("select self_set_plan('equipe', 'monthly')");
    await db.query("select self_cancel()");
    await db.query("select self_resume()");
    await db.exec("reset role");
    expect((await db.query("select subscription_status, cancel_at from profiles")).rows).toEqual([{ subscription_status: "active", cancel_at: null }]);
  });
  it("ne permet pas à l'utilisateur de s'ajouter à la liste d'autorisation", async () => {
    await db.exec("set role authenticated");
    await expect(db.query("insert into invoice_billing_accounts(user_id) values ($1)", [USER])).rejects.toThrow(/permission denied/);
  });
  it("interdit les RPC facture à un compte Stripe, même inscrit dans la liste", async () => {
    await db.query("insert into invoice_billing_accounts(user_id) values ($1)", [USER]);
    await apply();
    await db.exec("set role authenticated");
    for (const sql of ["select self_set_plan('equipe', 'monthly')", "select self_cancel()", "select self_resume()"]) {
  ...
  it("refuse les paramètres NULL plutôt que d'activer un plan invalide", async () => {
    await db.query("insert into invoice_billing_accounts(user_id) values ($1)", [USER]);
    await expect(db.query("select self_set_plan(null, 'monthly')")).rejects.toThrow(/Formule invalide/);
    await expect(db.query("select self_set_plan('equipe', null)")).rejects.toThrow(/Cycle invalide/);
  });
});
